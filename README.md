...
「資料審核」與「模型訓練 → 訓練資料」皆提供「管理資料分割」；跨集合批次警示也可直接開啟管理器。流程為設定比例／種子 → 預覽群組與類別分布 → 套用，或「套用並建立新資料版本」。只處理目前已核准圖片；分割不會改動標註與核准狀態。
...
神經網路模型的「參數設定 → 學習率策略」支援固定、Cosine 衰減、線性衰減，以及暖身輪數、最低學習率與排程預覽。TorchVision 另支援依 Validation 指標停滯降率，可設定耐心輪數及下降倍率；缺少 Validation 時拒絕此策略，不使用 Test 決定降率。Ultralytics 使用原生排程，其預览為趨勢示意，實際值以訓練記錄為準。
...
執行中的桌面程式可從「設定 → 更新與版本 → 檢查本機程式更新」，再按「更新並自動重新開啟」載入 2.6.0。本版未新增套件需求；更新不需要重新安裝模型。
