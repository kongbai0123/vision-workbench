...
執行中的桌面程式若偵測到待套用修改，頂部「設定」、設定內的「更新與版本」分類及「Vision Workbench 程式」卡片會同步顯示紅點。進入該頁可查看檔案與阻擋原因；按一次「套用更新並重新啟動」便會保存目前工作、驗證程式並載入 2.7.3，不再開啟另一個更新視窗。本版未新增套件需求；程式與模型元件分開更新。
