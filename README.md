...
- SAM2 與 GrabCut 候選遮罩；SAM2 會在候選階段保守填補安全門檻內的微小孔洞並記錄位置，背景提示點及具語意孔洞維持不變，候選仍需由使用者接受後才會寫入標註。
...
- YOLO Seg 可先掃描完整固定資料版本；審核預檢與實際轉換共用孔洞拓樸，預設只在 Run 專屬副本修補安全門檻內的微小封閉孔洞，並保存圖片、標註、像素、座標、比例與 IoU 稽核。大孔洞、裂縫或多區塊仍會阻擋。
...
RT-DETR 與 YOLO26 Seg 使用 `.venv-models/ultralytics`。請先閱讀模型頁顯示的授權資訊，再於「設定 → 模型與元件」按「安裝必要元件」；選取模型本身不會建立環境或下載權重。YOLO26 Seg 預設使用官方預訓練權重微調，第一次開始訓練才下載 `.pt`，之後使用本機快取；也可明確選擇從隨機權重開始。Run 保存權重來源及 SHA256。RT-DETR 目前仍使用 YAML 架構從零訓練。
...
可指定群組集合、保留目前 Test（含同組圖片）、手動命名獨立情境群組。只有確認圖片彼此獨立時才使用「獨立圖片 · 類別平衡」策略；此策略仍合併完全相同圖片。Train 與 Validation 都必須包含所有已有標註的類別；預覽無法滿足時顯示缺失類別並停用套用，建立資料版本與開始訓練也會重新檢查（包含舊版本）。Test 可選，缺少類別時顯示評估警告。群組隔離與類別覆蓋無法兼得時，必須補充獨立來源或明確整理群組，不能只依比例硬分。智慧分割能降低資料洩漏與分布偏差，不能保證 Train／Validation Loss 相同。

少量資料的流程驗證可透過 `TrainingWorkspace.create_diagnostic_dataset_version(project_id, source_dataset_id)` 從既有固定版本建立另一本「流程驗證」副本。它依單類別拍攝批次的檔名時間順序分成連續 Train／Val／Test 區段，邊界各排除一張，保存來源版本、分配與排除清單；完全相同圖片跨集合時拒絕建立。此方法只適用於有明確來源、單類別且依序命名的圖片，不能保證場景獨立。Run、模型及評估記錄均標示 `data_quality.purpose=diagnostic`，不作正式泛化評估；專案標註、核准狀態、分割與來源 DatasetVersion 不會被修改。
...
Ultralytics 可設定梯度累積批次數（預設 1），有效批次為批次大小乘以累積數。介面顯示實際有效批次與成功權重更新次數；AMP 因非有限梯度跳過更新時不會誤計，也不捏造該輪沒有實際使用的 LR。Loss 使用整輪平均值，支援新舊 Ultralytics 格式；最終模型評估獨立保存，不再誤記為第 51／50 輪。

### 訓練監控與模型比較（2.5）

「單次訓練」先顯示評估摘要、固定執行設定，再以雙欄自動呈現模型已實際記錄的指標。沒有紀錄的 Validation Loss、學習率或硬體曲線不會被推測產生。

模型信心分數不是 Mask 覆蓋率；mAP50–95 也不是每張圖片的原尺寸 IoU。判斷精細輪廓時應把已核准標註和預測在原尺寸對照，分別檢查漏覆蓋與多覆蓋；整體 mAP 較高仍可能有單張的大塊漏分割。YOLO 模型候選推論使用明確 RGB 影像與原尺寸 Mask，避免色彩通道顛倒或將 letterbox 補邊直接拉伸回原圖。候選回到審核流程，仍需人工確認模型輸出。

學習率各參數組收進同一張圖；完整實測序列一致時合併圖例，不同數值或缺值時保留獨立線型。成功更新次數保留在設定摘要及 Epoch 清單，不增加重複圖表。流程驗證 Run 會醒目標示同拍攝批次跨集合的評估限制。
...
比較使用相同資料版本與評估分割。不同引擎的 Loss、Mask IoU 與語意 IoU 可能有不同定義，無法直接比較時會停用該指標疊圖並顯示原因，其餘可比較的指標繼續顯示。Validation 專供選模與訓練控制；Test 僅做最終評估，未設定時明確顯示尚未執行。詳細評估提供全資料集累積 Macro／Micro IoU、TP／FP／FN、偵測 AP／Precision／Recall 或分類混淆矩陣與每類 support；標註與預測都空白的類別顯示 N/A，不計入平均。評估來源同時列出 Test 是否獨立及跨集合的來源 session；流程驗證資料的分數不會標示成正式泛化結果。
...
執行中的桌面程式若偵測到待套用修改，頂部「設定」、設定內的「更新與版本」分類及「Vision Workbench 程式」卡片會同步顯示紅點。進入該頁可查看檔案與阻擋原因；按一次「套用更新並重新啟動」便會保存目前工作、驗證程式並載入 2.7.7，不再開啟另一個更新視窗。本版未新增套件需求；程式與模型元件分開更新。
...
- 智慧分割固定相同 SHA-256、拍攝批次與影片來源；Train 與 Validation 類別不完整時拒絕套用。診斷副本若刻意跨來源集合，評估會保留明確限制與重疊 session。

## 目前範圍

2.0 版已交付影像採集至實例分割模型預標註的本機閉環。正式產線部署、模型服務監控及毫米級量測校正仍需依實際相機、鏡頭、治具、光源與標定流程另外驗證。

目前已交付設定中心與模型目錄、TorchVision／Ultralytics 獨立環境、Mask R-CNN、Faster R-CNN、DeepLabV3、TorchVision 分類、RT-DETR 與 YOLO26 Seg 的訓練／評估流程。EfficientAD／PatchCore 保留為規劃項目，介面不會將尚未完成的 adapter 顯示為可安裝。範圍、介面與驗收清單見 [模型中心與設定中心規劃](docs/model-center-settings-plan.md)。
