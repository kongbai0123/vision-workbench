...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="vision-ui-") as directory:
        ...
        try:
            ...
            wait("document.querySelectorAll('#trainingRunDetail .metric-point').length===4")
            assert js("document.querySelector('#trainingRunDetail').innerText.includes('曲線不會左右掃視')")
            assert js("new Set([...document.querySelectorAll('#trainingRunDetail .metric-axis-label[text-anchor=middle]')].map(e=>e.textContent)).size===4")
