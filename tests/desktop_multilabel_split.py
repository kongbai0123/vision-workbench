...
def main():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        view=QWebEngineView();page=Page(view);view.setPage(page);view.resize(1440,1000);view.show();view.setUrl(QUrl(service.entry_url))
        def js(script):
        ...
        try:
            ...
            click('#previewSmartSplit');wait("document.querySelector('.split-manager-message').textContent.includes('只有 1') && !document.querySelector('#previewSmartSplit').disabled")
            ...
            click(".split-tab[data-panel='classes']")
            assert js("document.querySelector('#smartSplitDialog').innerText.includes('全部 圖片／物件')")
            assert js("!document.querySelector('.split-panel[data-panel=\"issues\"]').textContent.includes('來源群組不足')")
