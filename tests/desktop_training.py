...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="vision-training-ui-") as directory:
        ...
        try:
            ...
            wait("document.querySelector('#augmentationEffectiveTotal').textContent==='6 張原圖 · Train 每輪 6 筆'")
            assert js("document.querySelector('#augmentationSummary').textContent.includes('Train 每輪 6 筆輸入')")
            assert js("document.querySelector('#augmentationSummary').textContent.includes('Train 每輪另載入 4 次隨機增強')")
