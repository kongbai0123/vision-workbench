...
from .training_engine import ENGINE_KEY, atomic_json, predict, read_json
from .maskrcnn_engine import ENGINE_KEY as MASKRCNN_KEY
...
from .yolo_compatibility import analyze_manifest
from .split_quality import split_class_coverage, split_purpose
...
class TrainingWorkspace:
    ...
    def readiness(self, project_id):
        with self.store.connection(project_id) as db:
            revision = db.execute('SELECT revision FROM project').fetchone()[0]
        with self.lock:
            cache = getattr(self, '_readiness_cache', {})
            cached = cache.get(project_id)
            if cached and cached['project_revision'] == revision:
                return deepcopy(cached)
        report = self._calculate_readiness(project_id)
        with self.lock:
            self._readiness_cache = cache
    ...
    def create_dataset_version(self, project_id, augmentation=None):
        ...
        return self.dataset(project_id, dataset_id, persist=True)

    def dataset(self, project_id, dataset_id, *, persist=False):
        ...
        if not hasattr(self, 'artifacts'):
            self.artifacts = ArtifactRepository()
        return self.artifacts.dataset_summary(path, lambda manifest: self._dataset_summary(project_id, dataset_id, manifest), persist=persist)

    @staticmethod
    def _dataset_summary(project_id, dataset_id, manifest):
    ...
    def start_run(self, project_id, dataset_id, config, request_id=None):
        if request_id is not None and (not isinstance(request_id, str) or not 1 <= len(request_id) <= 128):
            raise ValueError('request_id 必須為 1–128 字元')
        with self.lock, exclusive_file_lock(self.store.directory(project_id) / '.training-admission.lock'):
            if request_id is not None:
                for previous in self.list_runs(project_id):
                    if previous.get('request_id') == request_id:
                        if previous.get('request_payload') != {'dataset_id': dataset_id, 'config': config}:
                            raise ValueError('相同 request_id 不可用於不同訓練設定')
                        return previous
    ...
    def _start_run_locked(self, project_id, dataset_id, config, request_id=None):
        ...
        with self.lock:
            if any(item.get('status') in {'queued', 'preparing', 'running', 'stopping'} for item in self.list_runs(project_id)):
                raise ValueError('此專案已有訓練正在執行')
            ...
            kwargs['env'] = {**os.environ, 'VISION_WORKBENCH_RESOURCE_ROOT': str(getattr(self, 'root', project_runs.parent))}
            if os.name == "nt":
            ...
            try:
                ...
                atomic_json(run_dir / 'control' / 'terminal.json', {
                    'status': 'failed', 'message': '無法啟動訓練程序', 'error': str(exc),
    ...
    def _reap(self, project_id, run_id, process):
        ...
        with self.lock:
            ...
            # Closing waits for this short catalog transaction, never the worker.
            try:
                self._sync_catalog(project_id)
            except Exception:
                import logging
                logging.exception('Could not refresh completed run catalog: %s/%s', project_id, run_id)
    ...
    def run(self, project_id, run_id):
        ...
        heartbeat = path.parent / 'heartbeat.json'
        if heartbeat.is_file():
            run['heartbeat'] = read_json(heartbeat)
            run['heartbeat']['age_seconds'] = max(0, time.time() - run['heartbeat'].get('time', 0))
            run['heartbeat']['stale'] = run['heartbeat']['age_seconds'] > 15 and run.get('status') in {'queued', 'preparing', 'running', 'stopping'}
            if run['heartbeat']['stale']:
                run['health_warning'] = '訓練心跳已逾時；尚未證實程序停止，請檢查日誌與資源狀態'
        metadata = path.parent / 'worker.json'
        if metadata.is_file():
            run.update(read_json(metadata))
        active = {'queued', 'preparing', 'running', 'stopping'}
    ...
    def list_runs(self, project_id):
        ...
        rows = [self.run(project_id, item.name) for item in parent.iterdir()
    ...
    def create_predictions(self, project_id, model_id, asset_ids=None, progress=lambda _message, _percent=None: None):
        model = self.model(project_id, model_id)
        ...
        if external_worker:
            ...
            try:
                ...
                with accelerator_lease(self.root, checkpoint=lambda: progress('等待運算資源')):
                    run_controlled([worker_python, "-m", worker, "--request", str(request_path),
                                    "--output", str(output_path)], progress,
                                   cwd=Path(__file__).resolve().parents[1], timeout=1800)
                if not output_path.is_file():
                    raise RuntimeError('推論程序未產生結果')
    ...
    def _sync_catalog(self, project_id):
        ...
        for directory in sorted(self.runs_dir(project_id).glob("R*")):
            ...
            if not path.is_file():
                continue
            try:
                row = self.run(project_id, directory.name)
    ...
    def overview(self, project_id):
        return {"readiness": self.readiness(project_id), "datasets": self.list_datasets(project_id),
                ...
                "storage": {"layout": "project-scoped", "root": str(self.store.directory(project_id)),
                            "database": {"checked": False, "reason": "完整檢查由維護作業執行"},
                            "catalog_issues": self.catalog_issues.get(project_id, [])}}

    def status(self, project_id):
        """Polling path: no manifests, annotation decoding or runtime probes."""
        return {'runs': self.list_runs(project_id)}
