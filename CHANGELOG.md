...
- 新增「快速估時（不訓練）」：只讀固定版本及近期相同設定的耗時紀錄；匹配資料、引擎、參數與環境指紋，不啟動模型測速或掃描全部圖片。
- 導入固定版本 tqdm，以實際 Batch／工作單位及各階段有效時間估算速度；組合快、慢 EMA 與累計平均，依近期預測誤差動態調整。
- 首個有效樣本提供快速初估，訓練、驗證、引擎最終檢查與保存分開計時；完整工作資料足夠時顯示全程剩餘，否則顯示明確的階段估算。
- 加入波動範圍、過期回報處理及長停頓後重建速度，Run 保存計時摘要供後續初估；舊 Run 仍可讀取，沒有資料庫遷移。
- 主程式與模型環境加入 tqdm 依賴檢查；新增不訓練的時鐘／callback／歷史回放及桌面驗證，公開包含較差情境的誤差結果，見 [時間估算文件](docs/time-estimation.md)。

## 2.19.0 — 2026-09-24

- 04–06 設定串接加入專案獨立草稿：保存增強、目前模型／資料版本、各模型輸入與設定分頁；切換專案和重啟後恢復。儲存採序列寫入、版本衝突檢查及交易歷史，未完成輸入可保留，發布前仍須通過驗證。
- 分割管理器恢復已套用的用途、比例、平衡目標、種子與鎖定設定；非法增強或模型參數顯示明確錯誤，避免沿用舊投影或啟用無效操作。
- 新增「驗證設定（不訓練）」：與啟動入口共用設定／資料相容性檢查，另驗證固定圖片 SHA-256，列出有效輸入量、設定批次、環境狀態與引擎未採用的配方欄位，不建立 Run 或啟動 worker。
- 修正內建像素基準的摘要誤計擴充、原生 SGD 未採用介面標示的 momentum，以及 Validation／Test adapter 接到增強配方時可能被擴充的問題。YOLO 資料準備以固定 manifest 配方為準。
- 固定版本發布使用一致快照、預期修訂檢查及跨程序鎖；分開記錄圖片資料修訂與標註修訂，避免審核、分割與配方來源不一致。
- 新增 migration 0006 的設定草稿與修訂歷史表，沿用升級前 SQLite 備份及交易遷移；補上 04–06 桌面、引擎契約、跨專案隔離與 12px 字體驗證。完整範圍見 [設定與儲存驗證](docs/setup-workflow-audit.md)。

## 2.18.4 — 2026-09-24

- 修正資料準備頁將 Train、Validation 與 Test 不同階段處理量相加為單一「資料事件」總數所造成的誤解。
- 改為並列原始圖片總數與 Train 每輪輸入筆數，清楚顯示原圖分割、額外增強載入及實際訓練輸入的關係。
- 將下方分布明確標示為「原始圖片分割」，說明隨機增強不會增加資料集張數或流入 Validation／Test。

## 2.18.3 — 2026-09-24

- 將資料準備、分割管理、相機參數、訓練圖表與設定頁的可見文字下限統一為 12px，改善資訊密集區域的閱讀性。
- 新增全域 CSS 字體下限測試，阻止後續介面樣式再次引入低於 12px 的文字。

## 2.18.2 — 2026-09-24

- 重整資料準備的增強區塊，將設定、轉換摘要、預覽、版本狀態及實際事件分配建立為清楚的操作流程。
- Train／Validation／Test 分別顯示原始圖片、新增事件與有效數量；明確說明擴充不會重新分割，新增事件只留在 Train。
- 新增「尚未固定／已固定於 DatasetVersion」狀態與直接建立固定資料版本的操作，避免把畫面草稿誤認為已套用到既有 Run。
- 說明每次載入會重新抽樣但不保證結果互不重複，並揭露 Ultralytics 與原生 TorchVision 對增強參數的支援差異。
- 訓練設定摘要改為同時顯示 Train 原圖與擴充事件數，補上事件投影單元測試與 DatasetVersion 實際保存桌面驗證。

## 2.18.1 — 2026-09-24

- 重整資料分割管理器為「選擇方式、調整比例、檢查並套用」三階段，主要設定集中於左側，預覽結果以總覽、注意事項、類別分布與手動調整分頁呈現。
- 用途改為可比較的說明卡片，比例欄位即時顯示合計；種子、保留 Test、手動不可拆分群組及寬鬆實驗收進進階操作。
- Train／Validation／Test 摘要與套用操作固定在容易找到的位置，設定變更後會明確要求重新預覽，阻擋原因集中於注意事項。
- 圖片層級平衡不再顯示不適用的「來源群組不足」警告；只有啟用來源隔離的正式評估會檢查並呈現來源群組條件。
- 新增來源警告篩選單元測試，並更新桌面分割流程與響應式版面驗證。

## 2.18.0 — 2026-09-24

- 移除資料審核頁的樣本獨立性提示、確認與「不再顯示」偏好，核准流程不再被獨立性判斷干擾。
- 原「已審核獨立樣本」改為「圖片層級平衡」，可直接預覽及套用，不要求額外確認；完全相同與手動同組圖片仍不會拆到不同集合。
- 資料準備、訓練監控與資料版本改用圖片層級平衡語意，不再保存或宣稱人工獨立確認；既有資料庫欄位僅保留相容性且不參與判斷。
- 更新單元與桌面流程測試，驗證舊確認失效時仍可完成圖片層級分割。

## 2.17.9 — 2026-09-24

- 樣本獨立性提醒新增「我已了解，後續不再顯示」核取方塊，偏好保存在目前電腦並於之後的審核頁隱藏橫幅。
- 隱藏提醒不會寫入樣本獨立確認，也不會略過資料準備階段的分割驗證，避免介面偏好改變訓練證據。
- 補上提示偏好單元測試與桌面持久化流程驗證。

## 2.17.8 — 2026-09-23

- 資料審核新增「全部標註狀態／有標註／無標註」分類，可與審核狀態、原因及檔名同時篩選。
- 審核卡片直接標示有無標註；全選與批次核准只作用於目前分類結果。
- 補上純篩選單元測試與桌面核准流程驗證。

## 2.17.7 — 2026-09-23

- 桌面版更新提醒改為所有執行模式每 2.5 秒檢查一次，並在頁面初始化後補送狀態；程式檔案有修正時，設定按鈕與更新頁會及時顯示紅點。
- 更新紅點加入低調脈動效果，並遵守系統的減少動態效果偏好。
- 標註頁重建影像清單時保留捲動位置與鍵盤焦點；目前圖片若在視窗外，只移動到剛好可見的位置，不再跳回清單最前面。
- 新增捲動位置單元測試及桌面工作流程回歸驗證。

## 2.17.6 — 2026-09-23

- 資料審核新增獨立的「永久刪除」，可批次刪除目前專案中的圖片、標註與修訂紀錄，不再需要把「排除訓練」當成刪除操作。
- 永久刪除確認視窗列出標註物件與已核准圖片數，並明示無法從審核垃圾桶還原；外部原始檔與既有固定訓練版本維持保留。
- 補上審核頁永久刪除、垃圾桶不含已刪項目及版面寬度的桌面回歸驗證。
- 修正 Windows 在回覆錯誤方法或無效小型上傳時可能將 400／405 回覆轉為連線重設的問題。

## 2.17.5 — 2026-09-18

- 模型試跑與標註比對面板各自加上模型下拉選單，與左側清單雙向同步；不必再回頭猜「現在用的是哪一個版本」。
- 模型清單的選定列標示「使用中」並提供 `aria-pressed`，清單上方說明選取會套用到試跑、比對與匯出。
- 試跑／比對結果上方固定標示產生它的模型版本，選取換成別的版本時顯示提示；執行中與完成訊息都寫出模型版本 ID。

## 2.17.4 — 2026-09-18

- 修正桌面版無法從檔案總管把 `.pt` 權重拖入「匯入外部模型」對話框的問題：原生拖放只路由到採集頁的匯入區，對話框收不到檔案。
- 原生拖放改為可註冊的區域清單，最後開啟的區域優先接手，對話框開啟時採集頁的匯入區不會再被標示或觸發。
- 桌面版拖入的權重沿用原始路徑直接複製匯入，不經過上傳暫存區；瀏覽器模式維持串流上傳與一次性代碼。
- 提示訊息改以 top layer 顯示，對話框開啟時不再被遮罩蓋住；每次顯示都會重新置頂。

## 2.17.3 — 2026-09-18

- 模型試跑的預測標籤改以畫面像素繪製，不再隨原始影像縮放而縮小，字體下限固定為 12px。
- 標籤背景會依文字尺寸調整，並在影像邊緣自動換位及限制範圍，避免文字或背景遭裁切。

## 2.17.2 — 2026-09-18

- 模型匯入在上傳權重前重新檢查 Ultralytics 元件，避免先傳檔案後才發現執行環境未安裝。
- 匯入端強制刷新元件狀態，剛完成安裝時不再沿用最長五分鐘的舊快取。
- 元件首次載入檢查逾時由 30 秒放寬為 90 秒，涵蓋 Torch 與 Ultralytics 第一次初始化設定的時間。

## 2.17.1 — 2026-09-18

- 外部模型匯入改用瀏覽器檔案選擇器，不再要求使用者輸入或知道完整檔案路徑。
- 新增 `.pt` 權重拖曳區、檔名與容量回饋，以及副檔名、空檔和單檔限制檢查。
- 瀏覽器以串流方式將權重送至本機一次性暫存區；驗證工作領取後立即失效，成功或失敗都會清除暫存副本。
