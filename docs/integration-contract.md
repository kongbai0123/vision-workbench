...
DatasetVersion: immutable `{dataset_version_id,project_id,project_revision,classes,class_mapping,assets:[{asset_id,image_file,sha256,annotation_revision,annotation_sha256,split,batch_id,shapes,objects}]}`. Each image is copied and hash checked before atomic publication.
