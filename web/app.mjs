...
const metricLabels={'train/loss':'Train loss','val/mean_iou':'Validation mIoU','val/box_mean_iou':'Validation Box IoU','val/recall_50':'Validation Recall@0.5','val/mean_dice':'Validation Dice'};
const metricColors={'train/loss':'#efb35b','val/mean_iou':'#55d4bd','val/box_mean_iou':'#55d4bd','val/recall_50':'#8ebdff','val/mean_dice':'#c795f5'};
...
function renderTrainingPlots(run){const rows=state.trainingMetrics?.run?.run_id===run.run_id?state.trainingMetrics.metrics||[]:[];const root=element('div',undefined,'training-plots');const loss=metricChart('訓練損失',rows,['train/loss'],run.config?.epochs),scores=metricChart('驗證指標',rows,['val/mean_iou','val/box_mean_iou','val/recall_50','val/mean_dice'],run.config?.epochs,{fixedUnit:true});if(loss)root.append(loss);if(scores)root.append(scores);if(!root.childElementCount)root.append(element('p',activeRunStates.has(run.status)?'等待第一個 Epoch 指標…':'此執行沒有可繪製的 Epoch 指標。','metric-empty'));else root.append(element('p','座標範圍固定於整個 Run；曲線不會左右掃視。移到數值點可查看精確值。','metric-chart-note'));return root}
...
function renderModelDetail(model){const root=$('modelDetail');root.replaceChildren();if(!model){const empty=element('div',undefined,'report-empty');empty.append(element('span','◇'),element('h3','尚無可用模型'),element('p','完成一次訓練與評估後，模型會連同資料來源出現在這裡。'));root.append(empty);return}
  ...
  const test=model.test||model.validation||{},values=test.box_mean_iou!==undefined?[['Test Box IoU',Number(test.box_mean_iou).toFixed(3)],['Recall@0.5',Number(test.recall_50).toFixed(3)]]:test.mean_dice!==undefined?[['Test mIoU',Number(test.mean_iou).toFixed(3)],['Test Dice',Number(test.mean_dice).toFixed(3)]]:[['Test Mean IoU',Number(test.mean_iou||0).toFixed(3)]];values.push(['測試圖片',number(test.images)],['類別數',number(model.classes?.length)]);const score=element('div',undefined,'run-metrics');for(const [label,value]of values){const item=element('div',undefined,'run-metric');item.append(element('span',label),element('b',value));score.append(item)}root.append(score);
  const facts=element('dl',undefined,'model-facts');for(const [label,value]of [['來源 Run',model.run_id],['訓練資料',model.dataset_version_id],['類別',(model.classes||[]).join('、')],['模型能力','訓練／評估／預標註／匯出'],['建立時間',date(model.created_at)]])facts.append(element('dt',label),element('dd',value));root.append(facts,element('p','模型與資料版本、類別映射和評估結果一起保存；專案後續修改不會回寫此模型。','readiness-item'));
