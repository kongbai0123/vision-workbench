...
export class SplitManager {
  ...
  async open(projectId){
    ...
    this.number('重現種子','smartSplitSeed',42,0,2147483647,v=>this.options.seed=v);
  ...
  async runPreview(){this.plan=null;await this.work(async()=>{
    ...
    const unitLabel=plan.source_isolation?'不可拆分來源群組':'圖片分配單位';
    this.message.textContent=`${plan.blockers?.length?'無法套用：'+plan.blockers.map(x=>x.message).join('；'):`預覽：${plan.groups.length} 個${unitLabel}，將調整 ${plan.changed} 張。`} ${plan.warnings.join('；')||'分配可重現，請確認比例與多類別分布。'}`;
  })}
  renderGroups(groups){this.groups.replaceChildren();const table=node('table'),head=node('thead'),tr=node('tr');for(const t of ['分配單位','圖片','目前集合','建議集合','指定／鎖定'])tr.append(node('th',t));head.append(tr);table.append(head);const body=node('tbody');
    for(const group of groups){const row=node('tr');row.dataset.groupId=group.id;
      const name=node('td'),img=node('img');const asset=this.info?.assets?.find(a=>a.id===group.asset_ids[0]);if(asset){img.src=asset.url+'?thumbnail=1';img.alt='';img.loading='lazy';name.append(img)}const display=this.plan&&!this.plan.source_isolation&&group.images===1?(asset?.name||group.asset_ids[0]):(group.names||group.sources).join(' / ');name.append(document.createTextNode(display));
      const splitName={train:'Train',val:'Validation',test:'Test','未分配':'未分配'};const current=['train','val','test','未分配'].filter(s=>group.current?.[s]).map(s=>`${splitName[s]} ${group.current[s]}`).join(' / ');
      row.append(name,node('td',String(group.images)),node('td',current||'未分配'),node('td',splitName[group.proposed]||group.proposed||'尚未預覽'));
