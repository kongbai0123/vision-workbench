...
async function importExternalModel(){
  const body=element('div'),pathLabel=element('label','權重檔案（.pt）'),path=document.createElement('input'),browse=button('選擇檔案','secondary',()=>safe(async()=>{const paths=await nativeChoose('model');if(paths.length)path.value=paths[0]})),line=element('div',undefined,'inline'),nameLabel=element('label','模型名稱（選填）'),name=document.createElement('input'),trustLabel=element('label',undefined,'check-label'),trust=document.createElement('input');
  path.id='modelImportPath';path.placeholder='完整 .pt 檔案路徑';pathLabel.htmlFor=path.id;name.id='modelImportName';name.maxLength=100;nameLabel.htmlFor=name.id;trust.id='modelImportTrusted';trust.type='checkbox';trustLabel.append(trust,document.createTextNode('我確認權重來自可信任的來源'));
  line.append(path,browse);body.append(element('p','支援 Ultralytics 相容的 YOLO 偵測／實例分割與 RT-DETR 偵測權重。將複製至專案並驗證架構、類別與 CPU 推論。'),pathLabel,line,nameLabel,name,element('p','載入 .pt 可能執行其中的 Python 程式碼。請只選擇自己訓練或信任來源的權重。','field-note'),trustLabel,element('p','模型類別沿用權重中的名稱；預標註前請在專案建立相同類別。匯入不會建立假的訓練紀錄或評估分數。','field-note'));
  const result=await formDialog({title:'匯入外部模型',body,confirm:'驗證並匯入',eyebrow:'IMPORT MODEL',onSubmit:async()=>{
    if(!path.value.trim())throw Error('請選擇 .pt 權重檔案。');if(!trust.checked)throw Error('請先確認權重來源可信任。');
    return pollJob(await api(projectPath('/model-import'),'POST',{path:path.value.trim(),name:name.value.trim(),trusted:trust.checked}));
  }});
